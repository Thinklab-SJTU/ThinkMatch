...
class Sinkhorn(nn.Module):
    ...
    def forward_log(self, s, nrows=None, ncols=None, dummy_row=False):
        ...
        for b in range(batch_size):
            ...
            #         log_s = log_s - log_sum
            mask_row = torch.ones((nrows[b], ncols[b]), device=s.device, dtype=s.dtype)
            mask_row[-1,:] = 0
            mask_column = torch.ones((nrows[b], ncols[b]), device=s.device, dtype=s.dtype)
            mask_column[:,-1] = 0

            for i in range(self.max_iter):
                if i % 2 == 0:
                    log_sum = torch.logsumexp(log_s, 1, keepdim=True)
                    log_sum = log_sum * mask_row
                    log_s = log_s - log_sum
                else:
                    log_sum = torch.logsumexp(log_s, 0, keepdim=True)
                    log_sum = log_sum * mask_column
                    log_s = log_s - log_sum

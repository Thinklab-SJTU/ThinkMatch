...
from src.sparse_paddle import CSRMatrix3d, CSCMatrix3d
...
def to_sparse(x, dense_dim=1):
    ...
    x_typename = paddle.typename(x).split('.')[-1]
    sparse_tensortype = getattr(paddle.sparse, x_typename)

    indices = paddle.nonzero(x)[:, :len(x.shape) - dense_dim + 1]
...
def sbmm_diag(t1, t2):
    ...
    return sdd_bmm_diag_paddle(t1, t2)
...
def sdsbmm(t1, t2):
    ...
    if type(t1) == list or t1.is_sparse:
        result = sds_bmm_paddle(t1, t2)
    else:
        result = sds_bmm_paddle(t2.transpose((0, 2, 1)), t1.transpose((0, 2, 1))).transpose((0, 2, 1))
    return result
...
def sssbmm_diag(m1, m2):
    ...
    This is a wrapper function and does not support gradient.
    """
    if (type(m1) == list and type(m1[0]) == paddle.Tensor) or type(m1) == torch.Tensor:
        m1 = paddle2ssp(m1)
    if (type(m2) == list and type(m2[0]) == paddle.Tensor) or type(m2) == torch.Tensor:
        m2 = paddle2ssp(m2)
...
class SparseDenseDenseBMM(Function):
    ...
    This is a work around utilizing paddle.mm for sparse x dense -> dense. Forward and backward options are implemented.
    """
    @staticmethod
    def forward(ctx, t1, t2):
        ...
        assert t1.is_sparse != t2.is_sparse, 't1, t2 must be one sparse and one dense!'
        if t1.is_sparse:
            result = sdd_bmm_paddle(t1, t2)
        else:
            result = sdd_bmm_paddle(t2.transpose((0, 2, 1)), t1.transpose((0, 2, 1))).transpose((0, 2, 1))
        return result
    ...
    def backward(ctx, dm):
        ...
        if ctx.needs_input_grad[0]:
            dt1 = paddle.bmm(dm, d_t2.transpose((0, 2, 1)))
            dt1 = dense_to_sparse(dt1)
        if ctx.needs_input_grad[1]:
            dt2 = sdd_bmm_paddle(s_t1.transpose((0, 2, 1)), dm)
        return dt1, dt2


def sdd_bmm_paddle(s_t1, d_t2):
    """
    bmm (Batch Matrix Matrix) for sparse x dense -> dense. This function itself doesn't support gradient.
    with s_t1.shape = (b, x, s), d_t2.shape = (b, s, y), the output shape is (b, x, y)
    This is a work around utilizing paddle.mm for sparse x dense -> dense
    ...
    #outp = paddle.empty(batch_num, x, y, dtype=s_t1.dtype, device=device)
    for b in range(batch_num):
        _s_t1 = get_batches(s_t1, b)
        outp = paddle.mm(_s_t1, d_t2[b, :, :])#, out=outp[b, :, :])
    return outp.view(1, x, y)


def sdd_bmm_diag_paddle(t1, t2):
    """
    Perform bmm and diagonal for sparse x dense -> dense. The diagonalized result is returned in vector tensor.
    ...
    if t1.is_sparse:
        d_t1 = t1.transpose((0, 2, 1)).to_dense()
        outp = paddle.sum(d_t1.mul_(t2), dim=1)
    else:
        d_t2 = t2.transpose((0, 2, 1)).to_dense()
        outp = paddle.sum(d_t2.mul_(t1), dim=2)
    return outp


def sds_bmm_paddle(s_t1, d_t2):
    """
    bmm (Batch Matrix Matrix) for sparse x dense -> sparse. This function doesn't support gradient.
    And sparse tensors cannot accept gradient due to the limitation of paddle implementation.
    with s_t1.shape = (b, x, s), d_t2.shape = (b, s, y), the output shape is (b, x, y)
    This is a work around utilizing paddle.smm for sparse x dense -> sparse
    ...
    for b in range(batch_num):
        ...
        _outp = paddle.smm(_s_t1, _d_t2)  # CUDA version of smm is not implemented
        outp.append(_outp)

    return outp


def bilinear_diag_paddle(s_t1: CSRMatrix3d, d_t2: torch.Tensor, s_t3: CSCMatrix3d, device=None):
    """
...
def dense_to_sparse(d_t):
    ...
    indices = paddle.nonzero(d_t)
    if len(indices.shape) == 0:  # if all elements are zeros
        return paddle.sparse_coo_tensor([], [], d_t.shape, dtype=dtype, device=device, requires_grad=req_grad)
    indices = indices.t()
    values = d_t[tuple(indices[i] for i in range(indices.shape[0]))]
    return paddle.sparse_coo_tensor(indices, values, d_t.size(), dtype=dtype, device=device, requires_grad=req_grad)
...
def get_batches(s_t, b=None, device=None):
    ...
    if b is not None:
        ...
        outp = paddle.sparse_coo_tensor(_coo, _data, s_t.shape[1:3], dtype=_data.dtype, device=device)
    else:
        ...
        for b in range(batch_num):
            ...
            _coo = coo[1:3, idx].view(2, -1)
            _data = data[idx].view(-1)
            outp.append(paddle.sparse_coo_tensor(_coo, _data, s_t.shape[1:3], dtype=_data.dtype, device=device))
    return outp


def slicing_paddle(s_t, slice, preserve_dim=False):
    """
    A slicing function for paddle sparse tensors.
    ...
    coo = s_t._indices()
    data = s_t._values()
    idx_flag = paddle.ones(coo.shape[1], dtype=torch.uint8, device=device)
    for i in range(dim):
        s = slice[i]
        if s == -1:
            continue
    ...
    if not preserve_dim:
        ...
        if dim_flag.numel() == 0:
            coo = paddle.tensor([[0]], dtype=coo.dtype, device=device)
            shape = paddle.Size([1])
        else:
            coo = coo[:, idx]
            coo = coo[dim_flag, :]
            shape = paddle.Size(torch.tensor(s_t.shape)[dim_flag])
    else:
        coo = coo[:, idx]
        coo.mul_((slice == -1).type(coo.dtype).view(-1, 1))
        _dtype = paddle.int32
        shape = paddle.Size(torch.tensor(s_t.shape, dtype=_dtype, device=device) * (slice == -1).type(_dtype)
                           + paddle.ones(len(s_t.shape), dtype=_dtype, device=device) * (slice != -1).type(_dtype))
    data = data[idx]

    return paddle.sparse_coo_tensor(coo, data, shape, dtype=s_t.dtype, device=s_t.device)
...
def ssp2paddle(M, batch='dim', dtype=torch.float32, device=None):
    """
    Convert scipy.sparse matrix to paddle sparse matrix. Since scipy.sparse has a dimension limit of 2, list of matrices
    ...
    :param device: device
    :return: output paddle sparse matrix
    """
    ...
    if batch == 'list':
        ...
        for i in range(batch_num):
            ...
            data = _M.data
            outp.append(paddle.sparse_coo_tensor(coo, data, _M.shape, dtype=dtype, device=device))
    else:
        ...
        outp = paddle.sparse_coo_tensor(coo, data, torch.Size([batch_num] + list(_M.shape)), dtype=dtype, device=device)
...
def paddle2ssp(M):
    """
    Convert paddle sparse matrix to scipy.sparse matrix. Since scipy.sparse has a dimension limit of 2, batches are
    represented in list in the output.
    :param M: input paddle sparse matrix
...
if __name__ == '__main__':
    t = paddle.tensor([[[ 1,  2,  3,  4],
                       [11, 22, 33, 44]]])
    t = dense_to_sparse(t)
    s = slicing_paddle(t, torch.tensor((0, 0, 1)), preserve_dim=True)
    print(s.to_dense())


    from paddle.autograd import gradcheck
    input = (dense_to_sparse(paddle.randn(1, 20, 30, dtype=torch.double, requires_grad=True)),
             paddle.randn(1, 30, 40, dtype=torch.double, requires_grad=True))

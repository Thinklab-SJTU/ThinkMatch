...
def matching_accuracy(pmat_pred, pmat_gt, ns):
    ...
    assert numpy.all((pmat_pred == 0).asnumpy() + (pmat_pred == 1).asnumpy()), 'pmat_pred can only contain 0/1 elements.'
    assert numpy.all((pmat_gt == 0).asnumpy() + (pmat_gt == 1).asnumpy()), 'pmat_gt should only contain 0/1 elements.'
    assert numpy.all((P.ReduceSum()(pmat_pred, -1) <= 1).asnumpy()) and numpy.all((P.ReduceSum()(pmat_pred, -2) <= 1).asnumpy())
    assert numpy.all((P.ReduceSum()(pmat_gt, -1) <= 1).asnumpy()) and numpy.all((P.ReduceSum()(pmat_gt, -2) <= 1).asnumpy())

    match_num = 0
    total_num = 0
    acc = np.zeros(batch_num)
    for b in range(batch_num):
        acc[b] = (P.ReduceSum()(pmat_pred[b, :ns[b].asnumpy().item()] * pmat_gt[b, :ns[b].asnumpy().item()]) / P.ReduceSum()(pmat_gt[b, :ns[b].asnumpy().item()])).asnumpy().item()
        match_num += P.ReduceSum()(pmat_pred[b, :ns[b].asnumpy().item()] * pmat_gt[b, :ns[b].asnumpy().item()])
        total_num += P.ReduceSum()(pmat_gt[b, :ns[b].asnumpy().item()])
...
def matching_precision(pmat_pred, pmat_gt, ns):
    ...
    assert numpy.all((pmat_pred == 0).asnumpy() + (pmat_pred == 1).asnumpy()), 'pmat_pred can only contain 0/1 elements.'
    assert numpy.all((pmat_gt == 0).asnumpy() + (pmat_gt == 1).asnumpy()), 'pmat_gt should only contain 0/1 elements.'
    assert numpy.all((P.ReduceSum()(pmat_pred, -1) <= 1).asnumpy()) and numpy.all(
        (P.ReduceSum()(pmat_pred, -2) <= 1).asnumpy())
    assert numpy.all((P.ReduceSum()(pmat_gt, -1) <= 1).asnumpy()) and numpy.all(
    ...
    for b in range(batch_num):
        precision[b] = (P.ReduceSum()(pmat_pred[b, :ns[b].asnumpy().item()] * pmat_gt[b, :ns[b].asnumpy().item()]) / P.ReduceSum()(pmat_pred[b, :ns[b].asnumpy().item()])).asnumpy().item()
        match_num += P.ReduceSum()(pmat_pred[b, :ns[b].asnumpy().item()] * pmat_gt[b, :ns[b].asnumpy().item()])
        total_num += P.ReduceSum()(pmat_pred[b, :ns[b].asnumpy().item()])
...
def format_accuracy_metric(ps, rs, f1s):
    ...
    return 'p = {:.4f}v{:.4f}, r = {:.4f}v{:.4f}, f1 = {:.4f}v{:.4f}' \
        .format(np.mean(ps).asnumpy().item(),np.std(ps).asnumpy().item(), np.mean(rs).asnumpy().item(), np.std(rs).asnumpy().item(), np.mean(f1s).asnumpy().item(), np.std(f1s).asnumpy().item())
...
def format_metric(ms):
    ...
    """
    return '{:.4f}+-{:.4f}'.format(np.mean(ms).asnumpy().item(), np.std(ms).asnumpy().item())
    ...
    for b in range(batch_num):
        ...
        for i in range(num_clusters[b]):
            for j, k in combinations(range(num_clusters[b]), 2):
                ...
                sum += ((P.ReduceSum()(pred_i * gt_j) * P.ReduceSum()(pred_i * gt_k)) / P.ReduceSum()(pred_i) ** 2).asnumpy().item()
        for i in range(num_clusters[b]):
            for j, k in combinations(range(num_clusters[b]), 2):
                ...
                sum += ((P.ReduceSum()(gt_i * pred_j) * P.ReduceSum()(gt_i * pred_k)) / (P.ReduceSum()(pred_j) * P.ReduceSum()(pred_k))).asnumpy().item()

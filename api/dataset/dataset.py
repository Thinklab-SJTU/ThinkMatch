...
class SPair71k:
    def __init__(self, sets, obj_resize, problem='2GM'):
        """
        :param sets: 'train' or 'test'
        :param obj_resize: resized object size
        ...
        self.ann_files = open(os.path.join(self.SPair71k_layout_path, self.SPair71k_dataset_size, self.sets + ".txt"),
                              "r").read().split("\n")
...
class IMC_PT_SparseGM:
    ...
    def process(self):
        ...
        if not os.path.exists(img_file):
            total_cls = []
            for cls in cfg.IMC_PT_SparseGM.CLASSES['train']:
                total_cls.append(cls)
            for cls in cfg.IMC_PT_SparseGM.CLASSES['test']:
                total_cls.append(cls)
